...
@router.get(
    "/", status_code=HTTPStatus.OK, response_model=list[ActivityDB] | ActivityDB
)
def get_activity(
    ...
    if client_id:
        ...
        activity_db = session.scalars(
            select(ActivityModel).where(ActivityModel.client_id == client_id)
        )
        return activity_db
    ...
    if project_id:
        ...
        mask = (
            ...
            .join(ClientModel, ActivityModel.client_id == ClientModel.id)
            .where(ClientModel.project_id == project_id)
        )
...
def post_activity(q: ActivitySchema, session: Session = Depends(get_session)):
    ...
    activity_db = session.scalar(
        select(ActivityModel).where(ActivityModel.name == q.name)
    )
    if activity_db:
        raise HTTPException(
            status_code=HTTPStatus.CONFLICT, detail="activity already exists"
        )
...
def put_activity(
    activity_id: int, q: ActivitySchema, session: Session = Depends(get_session)
):
    "Modificar activity"
    client_id = q.model_dump()["client_id"]
    client_db = session.scalar(select(ClientModel).where(ClientModel.id == client_id))
    ...
    if activity_db.name != q.name:
        activity_same_name = session.scalar(
            select(ActivityModel).where(ActivityModel.name == q.name)
        )
